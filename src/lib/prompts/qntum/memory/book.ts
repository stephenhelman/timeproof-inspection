...
import { TIME_WINDOWS } from '../../../time-utils'
...
export function getBookMemoryModule(context: BookContext): string {
  ...
  // Address state
  if (address_collected && confirmed_address) {
    memory += `\nADDRESS: ${confirmed_address} — confirmed by homeowner. Use this if you need to reference their location.\n`
  } else {
    memory += `\nADDRESS: Not yet collected. Do NOT offer slots until you have the address.\n`
  }

  // Slot context — only relevant after address is collected
  if (address_collected) {
    ...
    if (time_preference !== 'any') {
      const timeWindowLabel = TIME_WINDOWS[time_preference].label
