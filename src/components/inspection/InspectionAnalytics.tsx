...
// REP + JORDAN ONLY. Clinical diagnosis analytics — severity, confidence,
// finding-type labels, the acknowledged list. This is deliberately NOT part of
// the homeowner-facing step flow: it lives behind its own sidebar button so a
// stray swipe past the slideshow can never surface severity scores in front of
// the homeowner.
...
export default function InspectionAnalytics({ inspectionId, initialData }: Props) {
  ...
  }, [inspectionId, structured]);

  if (loading) {
    return <p className="text-text-hint text-sm px-5 py-6">Loading analytics…</p>;
  }

  if (!structured || (!structured.overallSeverity && !(structured.zones?.length))) {
    return (
      <div className="px-5 py-6">
        <p className="text-text-secondary text-sm">
          No clinical analytics yet. Generate the diagnosis from the slideshow first.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3 p-5">
      <div className="bg-brand-navy/40 border border-brand-blue/30 rounded-xl px-4 py-2.5">
        <p className="text-text-accent text-xs font-semibold">Internal — rep &amp; Jordan only</p>
        <p className="text-text-secondary text-xs mt-0.5 leading-relaxed">
          Severity, confidence, and finding types. Never shown to the homeowner.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-2">
          {structured.zones.map((z, i) => (
            ...
            >
              ...
              <p className="text-text-secondary text-sm">{toTitleCase(z.findingType)}</p>
