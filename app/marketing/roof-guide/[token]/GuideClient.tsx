...
const FLAT_COMPARISON = [
  { feature: "Technology",     old: "BUR / modified bitumen",          current: "EPDM / TPO single-ply" },
...
export default function GuideClient({
  ...
  const shingleZonesWithCallouts: DiagramZone[] = SHINGLE_COMPONENT_ZONES.map((z) => ({
    ...z,
    callout:
      z.id === "flashing"
        ? config.zoneCallouts.flashing
    ...
    <>
      ...
      <section className="px-5 pt-16 pb-10 max-w-3xl mx-auto">
        ...
        <div className="mt-6 flex flex-wrap gap-3 justify-center">
          ...
          {issuesList.length > 0 && (
            <span className="inline-flex items-center gap-1.5 bg-bg-elevated border border-border rounded-full px-3 py-1.5 text-xs text-text-secondary">
              <span className="text-warning-text">■</span> {issuesList.length} issue{issuesList.length !== 1 ? "s" : ""} noted
            </span>
          )}
        </div>
      ...
      <section className="px-5 py-12 bg-bg-surface">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-text-secondary text-sm leading-relaxed mb-8 max-w-xl">
            By the time you see it inside, it&apos;s already been years in the making. El Paso&apos;s
            UV index ranks among the highest in the nation — heat cycling and 297+ sunny days
            per year accelerate degradation faster than manufacturer ratings account for.
          </p>
          ...
          <div className="mt-5 border-l-2 border-[#F06B30] pl-4">
            <p className="text-sm text-text-secondary leading-relaxed">
              <span className="text-[#F06B30] font-semibold">⚠ El Paso Climate Reality:</span>{" "}
              A standard shingle installed here realistically performs 10–15 years — not the 25 years
              on the box. That&apos;s why we install 50-year warranted architectural shingles rated for this climate.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-12">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-text-secondary text-sm leading-relaxed mb-8 max-w-xl">
            Most inspections only check the field shingles — sometimes the ridge. There are 5 zones
            where roofs actually fail, and most of the serious ones are invisible from the ground.
            Tap any zone to learn more.
          </p>
          <InteractiveDiagram
      ...
      <section className="px-5 py-12 bg-bg-surface">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { stat: "Top 3 UV Index", body: "El Paso ranks among the top 3 cities nationally. Manufacturer lifespan ratings are based on national averages — El Paso is not average." },
              { stat: "297 Sunny Days/Year", body: "vs. 205 nationally. Every one of those days is UV exposure breaking down granule adhesion on your roof surface." },
              { stat: "150–172°F Roof Surface", body: "While air temps hit 104°F, roof surfaces reach 150–172°F in summer. Daily heat cycling stresses every seam and granule bond." },
              { stat: "Up to 40% Shorter Lifespan", body: "A 3-tab shingle rated for 15–20 years realistically performs 10–15 years here. The climate matters more than the label." },
            ].map(({ stat, body }) => (
              <div key={stat} className="border border-border bg-bg-base rounded-xl p-5">
      ...
      {isShingle && (
        <section className="px-5 py-12">
          <div className="max-w-3xl mx-auto">
            <SectionLabel>Shingle Roof — What You&apos;re Actually Working With</SectionLabel>
            <TabGroup tabs={["Components", "Attic", "Old vs. New Tech", "What They Don't Tell You"]}>
              {(active) => (
                <>
                  {active === 0 && (
                    <div>
                      <p className="text-text-secondary text-sm leading-relaxed mb-6">
                        ...
                        Tap any zone to understand what it does — and why it matters.
                      </p>
                      <InteractiveDiagram
                  ...
                  {active === 1 && (
                    <div>
                      <p className="text-text-secondary text-sm leading-relaxed mb-6">
                        Most roofing companies never go into the attic. The attic is where the real
                        condition of your roof is visible. Tap any zone to understand what we look for.
                      </p>
                      <InteractiveDiagram
                  ...
                  {active === 2 && (
                    <div>
                      <p className="text-text-secondary text-sm leading-relaxed mb-6">
                        The gap between a roof installed 12–15 years ago and what&apos;s available now
                        is significant in every measurable category.
                      </p>
                      <div className="rounded-xl overflow-hidden border border-border">
                        <div className="grid grid-cols-3 bg-bg-elevated border-b border-border">
                          <div className="px-4 py-3 text-xs font-bold tracking-wider text-text-hint uppercase">Feature</div>
                          <div className="px-4 py-3 text-xs font-bold tracking-wider text-error-text uppercase">Old Standard</div>
                          <div className="px-4 py-3 text-xs font-bold tracking-wider text-success-text uppercase">Current</div>
                        </div>
                        {SHINGLE_COMPARISON.map((row, i) => (
                          <div
                            ...
                            className={`grid grid-cols-3 border-b border-border last:border-0 ${i % 2 === 0 ? "bg-bg-base" : "bg-bg-surface"}`}
                          >
                            <div className="px-4 py-3 text-sm font-semibold text-text-primary">{row.feature}</div>
                            <div className="px-4 py-3 text-sm text-error-text">{row.old}</div>
                            <div className="px-4 py-3 text-sm text-success-text">{row.current}</div>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {isFlat && (
        <section className={`px-5 py-12 ${isShingle ? "bg-bg-surface" : ""}`}>
          <div className="max-w-3xl mx-auto">
            <SectionLabel>Flat Roof — What You&apos;re Actually Working With</SectionLabel>
            <TabGroup tabs={["Components", "Old vs. New Tech", "What They Don't Tell You"]}>
              {(active) => (
                <>
                  {active === 0 && (
                    <div className="space-y-4">
                      {[
                        { label: "EPDM / TPO Membrane", body: "The primary waterproof layer. Susceptible to UV degradation, punctures, and seam failure over time." },
                        ...
                        { label: "Drains & Scuppers", body: "Drainage points that prevent ponding. Blocked drains are the number one cause of flat roof failure." },
                        { label: "Flashing & Edges", body: "Seals at perimeters, penetrations, and walls. Most active leak points originate here." },
                        { label: "Pitch Pockets", body: "Sealed openings around pipes and equipment. Require periodic maintenance — frequently neglected." },
                      ].map(({ label, body }) => (
                        <div key={label} className="border border-border bg-bg-surface rounded-xl p-4">
                  ...
                  {active === 1 && (
                    <div>
                      <p className="text-text-secondary text-sm leading-relaxed mb-6">
                        Flat roofing technology has evolved significantly. Modern single-ply membranes
                        outperform built-up systems in heat resistance, lifespan, and seam reliability.
                      </p>
                      <div className="rounded-xl overflow-hidden border border-border">
                        <div className="grid grid-cols-3 bg-bg-elevated border-b border-border">
                          <div className="px-4 py-3 text-xs font-bold tracking-wider text-text-hint uppercase">Feature</div>
          ...
          <div className="space-y-3">
            {DIFFERENTIATORS.map((item) => (
              <div key={item} className="flex items-start gap-3">
                <span className="shrink-0 text-success-text font-bold text-sm mt-0.5">✓</span>
      ...
      <div className="fixed bottom-0 inset-x-0 z-40 bg-bg-elevated border-t border-border px-4 py-3 flex gap-3 sm:max-w-lg sm:mx-auto sm:inset-x-auto sm:w-full" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}>
        ...
        >
          <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h4a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
          </svg>
          Save My Guide
        </a>
      </div>
