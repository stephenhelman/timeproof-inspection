...
export default async function BusinessCardPage({
  ...
  // Generate QR code pointing to this card page (including report param if present)
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden w-full max-w-sm">
        {/* Header with logo */}
        <div className="bg-brand-blue flex items-center justify-center py-4">
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="flex flex-col items-center gap-3 pb-1">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">Roofing Specialist</p>
            </div>
          </div>
          ...
          {(user.cardShowPhone || user.cardShowEmail || user.cardShowArea) && (
            <div className="flex flex-col gap-3 border-t border-gray-100 pt-4">
              {user.cardShowPhone && user.phone && (
                <a
                  href={`tel:${user.phone}`}
                  className="flex items-center gap-3 text-gray-700 text-sm hover:text-brand-blue transition-colors group"
                >
                  <span className="w-8 h-8 rounded-full bg-gray-50 group-hover:bg-brand-blue/10 flex items-center justify-center shrink-0 transition-colors">
                    <svg
                      className="w-4 h-4 text-gray-400 group-hover:text-brand-blue"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          {user.cardShowReportLink && reportUrl && (
            <a
              ...
              className="flex items-center gap-3 bg-brand-blue/10 hover:bg-brand-blue/15 border border-brand-blue/20 rounded-xl px-4 py-3 transition-colors"
            >
              <svg
          ...
          {user.cardShowQr && qrDataUrl && (
            <div className="flex flex-col items-center gap-2 pt-2 border-t border-gray-100">
              ...
              />
              <p className="text-gray-400 text-xs">Scan to save my contact</p>
            </div>
          )}
        ...
        <div className="border-t border-gray-100 py-3 flex items-center justify-center gap-1.5">
          ...
          <Link
            href="https://www.timeproofusa.com/"
            className="text-brand-blue hover:text-brand-navy text-xs font-semibold tracking-wide"
          >
            TimeproofUSA
          </Link>
        </div>
