...
import { getZoneForZip, getZipTier, SERVICE_ZONES, isDistanceZone } from "@/src/lib/service-zones";
...
import type {
  QualifyContext, ConversationTrack, QualifyLastMessageContext,
  BookContext, BookLastMessageContext,
  NurtureContext, NurtureLastMessageContext,
  RevivalContext, RevivalScenario, RevivalLastMessageContext,
  RescheduleContext, RescheduleReason, RescheduleLastMessageContext,
  FinanceContext, FinanceLastMessageContext,
...
function detectQualifyConversationTrack(messages: BotMessage[]): ConversationTrack {
  ...
  if (/not interested|leave me alone|stop texting|don't (want|need)|go away/.test(userText)) return "resistant";
  if (userMessages.length >= 2 && avgLength < 15) return "resistant";
  if (/leak|leaking|drip|missing shingle|crack|damage|hail|storm|water stain|sagging|rot|blow off|ice dam/.test(userText)) return "problem_aware";
  if (avgLength > 60) return "warm";
  return "problem_unaware";
}

function detectQualifyLastMessageContext(message: string): QualifyLastMessageContext {
  if (!message || message === "new_lead") return "none";
  const t = message.toLowerCase();
  if (/speak (to|with) (someone|a person|your manager)|call me back|want a human/.test(t)) return "escalation_needed";
  if (/insurance|claim|adjuster|deductible/.test(t)) return "insurance_mention";
  if (/other (company|roofer)|another (company|roofer)|got a (quote|bid)/.test(t)) return "competitor_mention";
  if (/how much|what('s| does) it cost|afford|financing|payment plan/.test(t)) return "financial_signal";
  if (/husband|wife|spouse|partner|not here|need to (ask|check with)/.test(t)) return "decision_maker_issue";
  if (/not (right now|a good time)|too busy|not ready|can't/.test(t)) return "timing_objection";
  if (/leak|drip|missing|damage|hail|storm|crack|stain|rot|blow off/.test(t)) return "problem_confirmed";
  if (/last (time|company)|bad experience|they (messed|screwed)/.test(t)) return "negative_experience";
  if (/^(yes|yeah|yep|sure|sounds good|ok|okay|go ahead|let'?s do it)\.?$/.test(t.trim())) return "expressed_interest";
  if (/no (issues|problems|damage)|looks fine|everything('s| is) (fine|ok)/.test(t)) return "problem_denied";
  return "stalling";
}
...
async function handleQualifyWebhook(ctx: CentralWebhookContext): Promise<void> {
  ...
  const rawSource = (lead as unknown as Record<string, unknown>).source as string ?? null;
  ...
  const context: QualifyContext = {
    ...
    has_strong_signal: activeIssues.length > 0 || lead.roofAge === "20+ years" || lead.lastInspected === "Never, as far as I know",
    source,
  ...
  const botMessages = currentMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp }));
  const rawResponse = await runBot(systemPrompt, botMessages);
  if (rawResponse === null) return;

  const signal = rawResponse.includes("[QUALIFIED]") ? "QUALIFIED"
    : rawResponse.includes("[NOT_INTERESTED]") ? "NOT_INTERESTED"
    : rawResponse.includes("[SOFT_CLOSE]") ? "SOFT_CLOSE"
    : null;
  ...
  if (signal === "QUALIFIED") {
    await transitionLead(lead.id, ghlContactId, "sr_qualifying", "sr_qualified", "NEW", "booking", {
      sr_qualify_status: "qualified", sr_bot_stage: "booking",
    });
    ...
    await transitionLead(lead.id, ghlContactId, "sr_qualifying", null, "NEW", "silent", { sr_bot_stage: "silent" });
...
function detectBookLastMessageContext(message: string): BookLastMessageContext {
  ...
  if (/speak (to|with) (someone|a person|manager)|call me|want a human/.test(t)) return "escalation_needed";
  if (/check (with|my)|need to (ask|check)|not sure (yet|about)/.test(t)) return "stall";
  if (/^(yes|yeah|yep|sure|confirmed|that works|perfect|ok|okay|sounds good)\.?$/.test(t) || /that work|confirmed|book it/.test(t)) return "confirmed";
...
async function handleBookWebhook(ctx: CentralWebhookContext): Promise<void> {
  ...
  if (trigger === "stall_exhausted") {
    await transitionLead(lead.id, ghlContactId, "sr_booking", "sr_follow_up", "REVIVAL_PENDING", "revival", {
  ...
  if (trigger === "inbound_sms" && isCancellation(inboundMsg)) {
    const activeInspection = await prisma.inspection.findFirst({ where: { leadId: lead.id, status: "scheduled" } });
    if (activeInspection) {
      await prisma.inspection.update({ where: { id: activeInspection.id }, data: { status: "cancelled", repNotes: "Cancelled via SMS" } });
      await transitionLead(lead.id, ghlContactId, "sr_booking", "sr_cancelled", "DEMO_NOT_SOLD", "silent", { sr_status: "DEMO_NOT_SOLD", sr_bot_stage: "silent" });
      const fn = lead.customerName.trim().split(/\s+/)[0];
  ...
  if (!existingLock || trigger === "qualified_handoff" || trigger === "stall_followup") {
  ...
  const triggerMap: Record<string, Trigger> = { qualified_handoff: "qualified_handoff", stall_followup: "stall_followup" };
  ...
  const context: BookContext = {
    ...
    zone: leadZone ?? "",
    message_history_count: currentMessages.length,
    last_message_context: detectBookLastMessageContext(lastUserMsg?.content ?? inboundMsg),
  ...
  if (bookTrigger === "qualified_handoff" && messages.length === 0) {
    const openerRaw = await runBot(systemPrompt, [{ role: "user", content: "qualified_handoff", timestamp: new Date().toISOString() }]);
    if (openerRaw !== null) {
  ...
  if (bookTrigger === "stall_followup") {
    ...
    const botMessages = stalledMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp }));
  ...
  const bookedMatch = rawResponse.match(/\[BOOKED:\s*(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})\]/);
  ...
  if (bookedMatch) {
    ...
    if (validation.ok) {
      ...
      if (lead.assignedUserId) {
        ...
        const dayLabel = new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric", timeZone: tz }).format(date);
        const timeLabel = new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit", hour12: true, timeZone: tz }).format(date);
...
function detectNurtureLastMessageContext(message: string): NurtureLastMessageContext {
  ...
  const t = message.toLowerCase();
  if (/not interested|leave me alone|stop texting|don't (want|need)|no thanks/.test(t)) return "not_interested";
  if (/when (could|can) you (come|get) out|schedule|book|inspection|worried|concerned/.test(t)) return "intent_signal";
  if (/leak|missing|crack|damage|hail|stain|sagging|rot/.test(t)) return "problem_mentioned";
  if (/looks fine|no issues|don't think|not sure|just curious/.test(t)) return "no_problem_aware";
...
async function addUsedInsightId(threadId: string, insightId: string): Promise<void> {
  ...
  if (!existing.includes(insightId)) {
    await prisma.botThread.update({ where: { id: threadId }, data: { metadata: { ...meta, usedInsightIds: [...existing, insightId] } } });
  }
}
...
async function handleNurtureWebhook(ctx: CentralWebhookContext): Promise<void> {
  ...
  const issuesNoticed = (rawLead.issuesNoticed as string | null ?? null)?.split(",").map(s => s.trim()).filter(Boolean) ?? null;
  const sourceRaw = (rawLead.guideSource as string) ?? (rawLead.source as string) ?? "organic";
  const srcMap: Record<string, NurtureContext["source"]> = { "facebook-guide": "facebook-guide", "door": "door", "card": "card", "organic": "organic" };
  ...
  if (trigger === "nurture_drip" && dripPosition !== null) {
    ...
    const insightUsed = rawResponse.match(/\[INSIGHT_USED:\s*([a-z_]+)\]/i)?.[1]?.toLowerCase() ?? null;
    await sendGhlSms(ghlContactId, stripAnySignals(rawResponse));
    await appendMessage(threadId, "assistant", rawResponse);
    ...
    if (rawResponse.includes("[INSPECTION_INTENT]")) {
      ...
      await transitionLead(lead.id, ghlContactId, "source_free_guide", "sr_dead", "DEAD", "silent", { sr_status: "DEAD", sr_bot_stage: "silent" });
  ...
  if (isNew || messages.length === 0) {
    const systemPrompt = assembleNurturePrompt(context);
    const openerRaw = await runBot(systemPrompt, [{ role: "user", content: "new_guide_lead", timestamp: new Date().toISOString() }]);
    if (openerRaw !== null) {
      const insightUsed = openerRaw.match(/\[INSIGHT_USED:\s*([a-z_]+)\]/i)?.[1]?.toLowerCase() ?? null;
      await sendGhlSms(ghlContactId, stripAnySignals(openerRaw));
      await appendMessage(threadId, "assistant", openerRaw);
      if (insightUsed) await addUsedInsightId(threadId, insightUsed);
    }
  ...
  const freshContext: NurtureContext = { ...context, message_history_count: freshMessages.length, used_insight_ids: await getUsedInsightIds(threadId) };
  const systemPrompt = assembleNurturePrompt(freshContext);
  const botMessages = freshMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp }));
  const rawResponse = await runBot(systemPrompt, botMessages);
  if (rawResponse === null) return;
...
function detectRevivalConversationTrack(messages: BotMessage[]): ConversationTrack {
  const userMessages = messages.filter(m => m.role === "user");
...
function detectRevivalLastMessageContext(message: string): RevivalLastMessageContext {
  ...
  if (/not interested|no thanks|stop|leave me alone/.test(t)) return "not_interested";
  if (/already (fixed|done|replaced)|someone else did/.test(t)) return "not_interested";
  if (/price|cost|afford|expensive|too much|budget/.test(t)) return "objection_price";
  if (/not (the right )?time|too busy|not ready|wait|later/.test(t)) return "objection_timing";
  if (/husband|wife|spouse|partner|need to (ask|check with)/.test(t)) return "objection_spouse";
  if (/other (company|contractor|roofer)|someone else/.test(t)) return "objection_competitor";
  if (/think|not sure|need to decide|get back/.test(t)) return "objection_need_to_think";
...
async function handleRevivalWebhook(ctx: CentralWebhookContext): Promise<void> {
  ...
  const rawLead = lead as unknown as Record<string, unknown>;
  const lastInspection = await prisma.inspection.findFirst({ where: { leadId: lead.id }, orderBy: { createdAt: "desc" } });
  const inspectionFindings = lastInspection?.findingsNotes ?? (lastInspection?.diagnosis ? JSON.stringify(lastInspection.diagnosis) : null);
  const daysSince = lastInspection?.createdAt ? Math.floor((Date.now() - lastInspection.createdAt.getTime()) / 86400000) : null;
  ...
  if (inspectionFindings && dispoOutcome === "demo_not_sold") lead_scenario = "report_complete_not_sold";
  else if (inspectionFindings && dispoOutcome === "no_show") lead_scenario = "report_complete_no_show";
  ...
  if (isNew || messages.length === 0) {
    const systemPrompt = assembleRevivalPrompt(buildContext([], "follow_up_triggered"));
    const openerRaw = await runBot(systemPrompt, [{ role: "user", content: "follow_up_triggered", timestamp: new Date().toISOString() }]);
    if (openerRaw !== null) {
      await sendGhlSms(ghlContactId, stripAnySignals(openerRaw));
      await appendMessage(threadId, "assistant", openerRaw);
    }
    if (!inboundMsg || inboundMsg === "follow_up_triggered" || trigger !== "inbound_sms") return;
  }
  ...
  const thread = await prisma.botThread.findUnique({ where: { id: threadId } });
  const currentMessages = (thread?.messages as BotMessage[]) ?? [];
  const systemPrompt = assembleRevivalPrompt(buildContext(currentMessages, inboundMsg));
  const botMessages = currentMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp }));
  const rawResponse = await runBot(systemPrompt, botMessages);
  if (rawResponse === null) return;
  ...
  if (rawResponse.toLowerCase().includes("if you know anyone") && !referral_seed_planted) {
    await prisma.botThread.update({ where: { id: threadId }, data: { metadata: { ...meta, referralSeedPlanted: true } } });
  }

  if (rawResponse.includes("[RE_QUALIFY]")) {
    await transitionLead(lead.id, ghlContactId, "sr_follow_up", "sr_recovered", "NEW", "booking", { sr_status: "NEW", sr_bot_stage: "booking" });
    await addGhlTag(ghlContactId, "sr_booking");
  } else if (rawResponse.includes("[DEAD]")) {
    await transitionLead(lead.id, ghlContactId, "sr_follow_up", "sr_dead", "DEAD", "silent", { sr_status: "DEAD", sr_bot_stage: "silent" });
  }
}
...
function detectRescheduleLastMessageContext(message: string): RescheduleLastMessageContext {
  ...
  if (/not (going to|gonna|want to)|don't want|no thanks|not interested/.test(t)) return "wont_rebook";
  if (/check (with|my)|need to (ask|check)|not sure yet/.test(t)) return "stall";
  if (/can('t| not)|won't work|not (available|good)|different (time|day)/.test(t)) return "slot_rejected";
  if (/^(yes|yeah|sure|ok|okay|confirmed|that works)\.?$/.test(t) || /that work|confirmed|book it/.test(t)) return "confirmed";
  return "none";
}

async function handleRescheduleWebhook(ctx: CentralWebhookContext): Promise<void> {
  const { lead, ghlContactId, trigger, inboundMsg } = ctx;
  ...
  const from_booking_stall = String(rawLead.rescheduleReason ?? "").includes("booking_stall") || Boolean((rawLead.ghlTags as string[] | undefined)?.includes("booking_stall_exhausted"));
  const rawReason = (rawLead.rescheduleReason as string) ?? "other";
  const validReasons: RescheduleReason[] = ["cancelled_by_homeowner", "one_legger", "time_constraint", "porched", "rep_schedule_conflict", "weather", "booking_stall_exhausted", "other"];
  const reschedule_reason: RescheduleReason = validReasons.includes(rawReason as RescheduleReason) ? rawReason as RescheduleReason : from_booking_stall ? "booking_stall_exhausted" : "other";
  ...
  if (isNew || messages.length === 0) {
    const systemPrompt = assembleReschedulePrompt(buildContext([], "reschedule_triggered"));
    const openerRaw = await runBot(systemPrompt, [{ role: "user", content: "reschedule_triggered", timestamp: new Date().toISOString() }]);
    if (openerRaw !== null) {
      await sendGhlSms(ghlContactId, stripAnySignals(openerRaw));
      await appendMessage(threadId, "assistant", openerRaw);
    }
    if (!inboundMsg || inboundMsg === "reschedule_triggered" || trigger !== "inbound_sms") return;
  }
  ...
  const reloadedThread = await prisma.botThread.findUnique({ where: { id: threadId } });
  const reloadedMessages = (reloadedThread?.messages as BotMessage[]) ?? [];
  const systemPrompt = assembleReschedulePrompt(buildContext(reloadedMessages, inboundMsg));
  const botMessages = reloadedMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp }));
  const rawResponse = await runBot(systemPrompt, botMessages);
  if (rawResponse === null) return;
  ...
  const rescheduledMatch = rawResponse.match(/\[RESCHEDULED:\s*(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})\]/);
  if (rescheduledMatch) {
    const [, dateStr, time] = rescheduledMatch;
    const [year, month, day] = dateStr.split("-").map(Number);
    const [hour, minute] = time.split(":").map(Number);
    const date = new Date(year, month - 1, day, hour, minute, 0, 0);
    const validation = await validateSlotBeforeConfirm(lead.id, date, time, zoneStr);
    if (validation.ok) {
      await confirmBooking(lead.id, ghlContactId, date);
      ...
      if (lead.assignedUserId) {
        await notifyRep(lead.assignedUserId, lead.id, `Reschedule confirmed for ${lead.customerName} — ${date.toLocaleDateString()}`);
      }
    }
  } else if (rawResponse.includes("[RE_QUALIFY]") || rawResponse.includes("[DEAD]")) {
    if (!has_pivoted) {
      await prisma.botThread.update({ where: { id: threadId }, data: { metadata: { ...threadMeta, hasPivotedToRevival: true } } });
    }
    if (rawResponse.includes("[RE_QUALIFY]")) {
      await transitionLead(lead.id, ghlContactId, "sr_reschedule", null, "NEW", "booking", { sr_status: "NEW", sr_bot_stage: "booking" });
      await addGhlTag(ghlContactId, "sr_booking");
    } else {
      await transitionLead(lead.id, ghlContactId, "sr_reschedule", "sr_dead", "DEAD", "silent", { sr_status: "DEAD", sr_bot_stage: "silent" });
    }
  }
}
...
function detectFinanceLastMessageContext(message: string): FinanceLastMessageContext {
  ...
  const t = message.toLowerCase();
  if (/speak (to|with) (someone|manager)|rate|term|apr|credit score/.test(t)) return "escalation_needed";
  if (/can('t| not) (afford|do it)|there'?s no (way|path)|just (can'?t|won't)/.test(t)) return "firmly_cant_proceed";
  if (/equity|home equity|heloc/.test(t)) return "exploring_heloc";
  if (/credit union|local bank|navy federal/.test(t)) return "exploring_credit_union";
  ...
  if (/open to|willing to|what about|could (we|that)|maybe/.test(t)) return "open_to_options";
  if (/found (a way|something)|we can|it'll work|figured/.test(t)) return "option_identified";
...
async function handleFinanceWebhook(ctx: CentralWebhookContext): Promise<void> {
  ...
  if (isNew || messages.length === 0) {
    ...
    const openerRaw = await runBot(assembleFinancePrompt(ctx2), [{ role: "user", content: "credit_fail_triggered", timestamp: new Date().toISOString() }]);
    if (openerRaw !== null) {
      await sendGhlSms(ghlContactId, stripAnySignals(openerRaw));
      await appendMessage(threadId, "assistant", openerRaw);
    }
    if (!inboundMsg || inboundMsg === "credit_fail_triggered" || trigger !== "inbound_sms") return;
  }
  ...
  const rawResponse = await runBot(assembleFinancePrompt(financeCtx), reloadedMessages.map(m => ({ role: m.role as "user" | "assistant", content: m.content, timestamp: m.timestamp })));
  if (rawResponse === null) return;
  ...
  if (rawResponse.includes("[FINANCE_RETRY]")) {
    ...
    if (lead.assignedUserId) {
      ...
      await notifyRep(lead.assignedUserId, lead.id, `Finance path identified — homeowner exploring ${optionFound}. Follow up to support.`);
    }
    ...
    await transitionLead(lead.id, ghlContactId, "sr_credit_fail", "sr_dead", "DEAD", "silent", { sr_status: "DEAD", sr_bot_stage: "silent" });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const dripPosition: number | null = data.drip_position ? parseInt(data.drip_position) : null;

  if (!ghlContactId) {
    console.error("[central] no contact_id in payload", JSON.stringify(rawBody).slice(0, 500));
  ...
  if (!lead) {
    ...
    await logWebhookHit({ source: "ghl_central", payload: rawBody, success: false, error: "lead_not_found", idempotencyKey });
    return new Response("OK", { status: 200 });
  }
  ...
  if (trigger === "inbound_sms" && isOptOut(inboundMsg)) {
    await addGhlTag(ghlContactId, "sr_opted_out").catch(e => console.error(e));
    await transitionLead(lead.id, ghlContactId, null, "sr_dead", "DEAD", "silent", {
  ...
  const context: CentralWebhookContext = {
    lead, srLead, ghlContactId, trigger, inboundMsg, dripPosition, idempotencyKey, rawBody,
  };
  ...
  try {
    ...
    await logWebhookHit({ source: "ghl_central", payload: rawBody, success: false, error: message, leadId: lead.id, idempotencyKey });
