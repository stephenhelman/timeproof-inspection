...
export async function POST(
  ...
  // GHL writes fire regardless of whether a manager exists
  ...
  // No manager account exists yet — GHL side effects still fired above.
  if (!manager) {
    return NextResponse.json(
      { error: "No manager account found. Escalation was flagged in GHL, but no task could be assigned. Ask an admin to create a manager account." },
      { status: 400 },
    );
  }
