          ...
          {n < 3 && (
            <div
              className={`flex-1 h-0.5 transition-all ${
                n < step ? "bg-brand-blue" : "bg-border"
              }`}
            />
          )}
        </React.Fragment>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        sessionStorage.setItem("reg_verified_email", data.verifiedEmail);
        onSuccess();
      }
    } catch {
      setError("Something went wrong. Please try again.");
...
function Step3Password({ email }: { email: string }) {
  ...
  const canSubmit = name.trim() !== "" && passwordValid && passwordsMatch && !loading;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      sessionStorage.removeItem("reg_verified_email");
            ...
            <div className="flex flex-col gap-1.5 mt-1">
              {requirements.map((req) => (
                <div key={req.text} className="flex items-center gap-2 text-sm">
                  <span className={req.met ? "text-green-400" : "text-text-hint"}>
                    {req.met ? "●" : "○"}
                  </span>
                  <span className={req.met ? "text-green-400" : "text-text-hint"}>
                    {req.text}
